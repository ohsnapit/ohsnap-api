...
import { 
  reactionsByFidQuerySchema, 
  reactionsByFidResponseSchema, 
  reactionsByFidExamples, 
  reactionsByCastQuerySchema, 
  reactionsByCastExamples, 
  reactionsByTargetQuerySchema, 
  reactionsByTargetExamples 
} from '../schemas/reactions.js';

export const reactionRoutes = new Elysia({ prefix: '/v1' })
  // Reactions by FID route
  .get('/reactionsByFid', async ({ query }) => {
    return withSpan(
...
- Paginated: Use pageToken for loading more results`,
  ...
  .get('/reactionsByCast', async ({ query }) => {
    return withSpan(
...
- Paginated: Use pageToken for loading more results`,
  ...
  .get('/reactionsByTarget', async ({ query }) => {
    return withSpan(
    ...
    detail: {
      ...
      description: `Get all reactions to a cast's target URL.

**Parameters:**
- **url** (required): The URL of the target
- **reaction_type** (required): The type of reaction (Like or Recast)
