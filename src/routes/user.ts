...
import { 
  ...
  userResponseSchema, 
  userExamples, 
  userCastsQuerySchema, 
  userCastsResponseSchema, 
  userCastsExamples 
...
- Paginated: Use cursor for loading more results`,
  ...
  .get('/user/by-username', async ({ query }) => {
    return withSpan(
    ...
    detail: {
      ...
      description: `Fetches a user's profile by resolving their X username to fid (via cache only).
