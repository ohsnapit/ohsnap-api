...
import { 
  onchainSignersQuerySchema, 
  onchainSignersResponseSchema, 
  onchainSignersExamples, 
  onchainEventsQuerySchema, 
  onchainEventsResponseSchema, 
  onchainEventsExamples 
} from '../schemas/onchain.js';

export const onchainRoutes = new Elysia({ prefix: '/v1' })
  // Onchain signers by FID route
  .get('/onChainSignersByFid', async ({ query }) => {
    return withSpan(
...
- Filtered: Get events for a specific signer key`,
  ...
  .get('/onChainEventsByFid', async ({ query }) => {
    return withSpan(
