...
import { getLinksByFid, getLinksByTargetFid } from '../services/http.js';
import { startTimer, logServiceMethod, logError } from '../utils/logger.js';
import { withSpan, addBreadcrumb } from '../utils/tracing.js';
import { 
  linksByFidQuerySchema, 
  linksByFidResponseSchema, 
  linksByFidExamples, 
  linksByTargetFidQuerySchema, 
  linksByTargetFidResponseSchema, 
  linksByTargetFidExamples 
} from '../schemas/links.js';

export const linkRoutes = new Elysia({ prefix: '/v1' })
  // Links by FID route
  .get('/linksByFid', async ({ query }) => {
    return withSpan(
      'GET /v1/linksByFid',
      'http.server',
      async () => {
...
- Paginated: Use pageToken for loading more results`,
  ...
  .get('/linksByTargetFid', async ({ query }) => {
    return withSpan(
